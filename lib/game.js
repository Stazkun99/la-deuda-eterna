...
class Game {
  ...
  interaction(r, from, action, to, amount = null, detail = '') {
    ...
    MatchSummary.interaction(r,from,to,item,this.now());
  }
  money(r, p, delta, meta = {}) {
    for (const q of this.team(r, p)) q.dinero += delta;
    if (delta && meta !== false) {
      this.interaction(r, delta < 0 ? p : meta.contraparte || 'Banca', meta.accion || (delta < 0 ? 'Pago' : 'Cobro'), delta < 0 ? meta.contraparte || 'Banca' : p, Math.abs(delta), meta.detalle || '');
      if(meta.multiplicador)r.interacciones.at(-1).multiplicador=meta.multiplicador;
    }
  }
  ...
  completedGroups(r){
    const current=[];
    for(const [name,ids] of Object.entries(GRUPOS_CADENAS)){
      const cells=ids.map(id=>r.tablero[id]),owner=this.player(r,cells[0].dueño);
      if(!owner||!cells.every(c=>this.sameTeam(owner,this.player(r,c.dueño))))continue;
      const key=name+':'+(owner.alianzaId||owner.id);current.push(key);
      if(!(r.gruposCompletos||[]).includes(key)){
        this.interaction(r,owner,'Cadena completa','La mesa',null,cells.map(c=>c.nombre).join(' + '));
        Object.assign(r.interacciones.at(-1),{casillas:ids,multiplicador:ids.length});
      }
    }
    r.gruposCompletos=current;
  }
  publish(r) { if(r.enJuego){this.completedGroups(r);MatchSummary.observe(this,r,this.now());}r.actualizado = this.now(); this.emit(r.codigo, 'actualizarEstado', this.state(r)); }
  ...
  join(socketId, data) {
    ...
    if (r.enJuego && !r.pausa && r.jugadores[r.turnoActual]?.id === p.id) r.limiteTurno = Math.max(r.limiteTurno, this.now() + 60_000);
  ...
  action(socketId, event, data = {}) {
    ...
    if(event==='prepararRevancha'){
      requireRule(p.esLider && r.finalizada && !r.enJuego, 'Solo el anfitrión puede preparar la revancha al terminar.');
      requireRule(data.resultadoId === (r.resultado?.id || null), 'Ese resultado ya no corresponde a la partida.');
      r.ultimoResultado=r.resultado;
      for(const q of r.jugadores){this.resetPlayer(q);delete q.dadoInicial;}
      Object.assign(r,{tablero:copy(TABLERO),enJuego:false,finalizada:false,resultado:null,pendiente:null,fase:'espera',turnoActual:0,turnoId:r.turnoId+1,limiteTurno:null,continuacion:null,pausa:null,barreraProteccionista:false,mazos:{},descuento:false,monopolizado:false,gruposCompletos:[],interacciones:[],registro:[],ultimaCarta:null,ultimaTirada:null,inicioPartida:null,inicioAnimacionHasta:null,estadisticas:null});
      this.log(r,'Revancha preparada. Conserváis la sala y los personajes; cada jugador vuelve a tirar su dado inicial.');
      this.publish(r);return;
    }
    if (event === 'pausarPartida') {
      requireRule(r.enJuego && !p.enQuiebra, 'La partida no está activa.');
      requireRule(typeof data.pausar === 'boolean', 'Indica pausa o reanudación.');
      if (data.pausar && !r.pausa) { r.pausa = {desde:this.now(), jugador:p.nombre}; this.log(r, p.nombre + ' ha pausado la partida.'); }
      else if (!data.pausar && r.pausa) {
        const delta=this.now()-r.pausa.desde;
        if(r.estadisticas)r.estadisticas.pausaMs+=delta;
        r.limiteTurno+=delta;
        if(r.pendiente?.vence)r.pendiente.vence+=delta;
        if(r.desconectadoDesde!=null)r.desconectadoDesde=this.now();
        r.pausa=null;this.log(r,p.nombre+' ha reanudado la partida.');
      }
      this.publish(r);return;
    }
    requireRule(!r.pausa, 'La partida está pausada. Reanúdala para continuar.');
    if (event === 'tirarDadoInicial') {
      requireRule(!r.enJuego && !r.finalizada && !!p.personaje, 'Elige tu personaje antes de tirar.');
      requireRule(!p.dadoInicial, 'Ya has realizado tu tirada inicial.');
      requireRule(!r.inicioAnimacionHasta || this.now()>=r.inicioAnimacionHasta, 'Espera a que termine la tirada anterior.');
      p.dadoInicial=this.dice();p.dinero=5000+p.dadoInicial*200;
      r.inicioAnimacionHasta=this.now()+1400;
      r.ultimaTirada={id:randomUUID(),tipo:'inicial',jugador:p.nombre,jugadorId:p.id,dados:[p.dadoInicial],total:p.dadoInicial};
      this.log(r,`${p.nombre} sacó ${p.dadoInicial}: empieza con $${p.dinero}.`);
      this.publish(r);return;
    }
    if (event === 'seleccionarPersonaje') {
      requireRule(!r.enJuego && !r.finalizada, 'El personaje se elige antes de empezar la partida.');
    ...
    if (event === 'iniciarPartida') {
      requireRule(p.esLider && !r.enJuego && !r.finalizada, 'Solo el anfitrión puede iniciar la partida.');
      ...
      if(r.jugadores.some(q=>q.dadoInicial)) {
        requireRule(r.jugadores.filter(q=>q.conectado).every(q=>q.dadoInicial), 'Cada jugador debe tirar su dado inicial.');
        requireRule(this.now()>=(r.inicioAnimacionHasta||0),'Espera a que termine la última animación.');
      }
      ...
      r.registro = []; r.interacciones = []; r.ultimaCarta = null; r.ultimaTirada = null; r.gruposCompletos=[];
      MatchSummary.start(r,this.now());
      const tiradas = r.jugadores.map(q => {
        this.resetPlayer(q);
        const dado = q.dadoInicial || this.dice(); delete q.dadoInicial; q.dinero = 5000 + dado * 200;
  ...
  beginTurn(r) {
    ...
    r.limiteTurno = (r.pausa?.desde ?? this.now()) + 180_000;
  ...
  debit(r, p, amount, recipient = null, detail = '') {
    ...
    if (amount) {
      this.interaction(r, p, detail.startsWith('Renta') ? 'Pago de renta' : 'Pago', recipient || 'FMI', amount, detail);
      const cell=r.tablero[p.posicion];
      if(detail.startsWith('Renta')&&cell?.precio)r.interacciones.at(-1).multiplicador=amount/cell.precio;
      if(detail==='Fuga de capitales')r.interacciones.at(-1).multiplicador=amount/1000;
    }
  }
  ...
  land(r, p, position) {
    ...
    if (c.tipo === 'propiedad') {
      ...
      if (c.region === 'sur') {
        ...
        if (own) {
          if (!r.barreraProteccionista && !p.industriasCerradas && !p.turnosPerdidos) this.money(r, p, this.rent(r, c), {accion:'Cobro de exportación',detalle:c.nombre,multiplicador:this.rent(r,c)/c.precio});
  ...
  card(r, p, type) {
    ...
    if (type === 'solidaridad') {
      ...
      switch (c.id) {
        ...
        case 2: case 4: p.posicion = 39; p.industriasCerradas = false; this.payment(r, p, this.interest(p, 0.15), 'Intereses FMI · ' + c.titulo); break;
        ...
        case 10: for (const q of active) { q.posicion = 39; q.industriasCerradas = false; this.debit(r, q, this.interest(q), null, 'Intereses FMI'); } break;
  ...
  finishVote(r) {
    ...
    if (voters.length >= 2) {
      const id = randomUUID();
      for (const p of voters) this.debit(r, p, this.interest(p), null, 'Intereses FMI al formar alianza');
  ...
  auction(r, p, data) {
    ...
    this.pending(r, 'subasta', p, { nombrePropiedad: c.nombre, base: Math.floor(this.investment(r, c) / 2), oferta: null, vence: this.now() + 10_000, pujas: [], anterior: previous });
    this.log(r, `Subasta de ${c.nombre}: 10 segundos sin nuevas pujas para cerrar.`);
  }
  bid(r, p, data) {
    const a = r.pendiente;
    ...
    a.vence=this.now()+10_000;
    a.pujas=[...(a.pujas||[]),{...a.oferta,nombre:p.nombre,fecha:this.now()}].slice(-20);
  }
  finishAuction(r) {
  ...
  finish(r, winner, reason) {
    ...
    const resumen=MatchSummary.finish(this,r,this.now());
    r.pausa=null; r.enJuego = false; r.finalizada = true; r.fase = 'finalizada'; r.pendiente = null;
    r.resultado = { id:randomUUID(), ganador: winner, motivo: reason, resumen };
    this.emit(r.codigo, 'finDeJuegoModal', r.resultado);
  }
  leave(socketId) {
    const { r, p } = this.current(socketId);
    if(r.enJuego){MatchSummary.observe(this,r,this.now()).jugadores[p.id].retirado=true;}
  ...
  tick() {
    ...
    for (const r of Object.values(this.rooms)) {
      ...
      if (r.pausa || !r.enJuego || !r.jugadores.some(p => p.conectado && !p.enQuiebra)) continue;
      const pending = r.pendiente;
      ...
      if (expired && !['subasta', 'votacion'].includes(r.fase)) {
        ...
        for (let i = 0; i < 8 && r.pendiente; i++) {
          ...
          if (d.tipo === 'pago') this.pay(r, p, { ...data, usarOro: !!d.oroPermitido && p.oro > 0 && p.dinero < d.monto });
