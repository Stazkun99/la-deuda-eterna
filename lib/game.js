...
class Game {
  ...
  publish(r) { if(r.enJuego)this.completedGroups(r);r.actualizado = this.now(); this.emit(r.codigo, 'actualizarEstado', this.state(r)); }
  ...
  action(socketId, event, data = {}) {
    ...
    if (event === 'tirarDadoInicial') {
      requireRule(!r.enJuego && !!p.personaje, 'Elige tu personaje antes de tirar.');
