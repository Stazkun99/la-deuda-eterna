...
class Game {
  constructor({ rooms = {}, emit = () => {}, now = Date.now, dice = () => randomInt(1, 7) } = {}) {
  ...
  money(r, p, delta) { for (const q of this.team(r, p)) q.dinero += delta; }
  gold(r, p, delta) { for (const q of this.team(r, p)) q.oro += delta; }
  ...
  action(socketId, event, data = {}) {
    ...
    if (event === 'iniciarPartida') {
      ...
      r.registro = []; r.ultimaCarta = null; r.ultimaTirada = null;
      for (const q of r.jugadores) { this.resetPlayer(q); q.dinero = this.dice() * 1000; }
      r.mazos = { solidaridad: this.shuffle(CARTAS_SOLIDARIDAD), condiciones: this.shuffle(CARTAS_CONDICIONES) };
      r.turnoActual = 0; this.beginTurn(r);
      ...
      if (event === 'responderVotoAlianza') this.vote(r, p, data);
        ...
        requireRule(!['votacion', 'subasta'].includes(r.fase), 'Espera a que termine la votación o la subasta.');
        switch (event) {
          ...
          case 'pedirPrestamo': this.loan(r, p); break;
          case 'pagarDeuda':
            ...
            { const amount = Math.min(5000, p.deudaPersonal); requireRule(p.dinero >= amount, 'No tienes efectivo suficiente.'); this.money(r, p, -amount); p.deudaPersonal -= amount; this.log(r, `${p.nombre} amortizó $${amount}.`); }
            break;
          ...
          case 'levantarBarrera':
            ...
            this.money(r, p, -2000); r.barreraProteccionista = false; this.log(r, `${p.nombre} levantó la barrera para todos.`); break;
  ...
  loan(r, p) {
    ...
    const amount = Math.min(5000, 30000 - borrower.deudaPersonal);
    borrower.deudaPersonal += amount; this.money(r, borrower, amount);
  ...
  debit(r, p, amount, recipient = null) {
    requireRule(Number.isSafeInteger(amount) && amount >= 0, 'Importe inválido.');
    this.money(r, p, -amount);
    if (recipient) this.money(r, recipient, amount);
  }
  ...
  pay(r, p, data) {
    ...
    if (data.usarOro) {
      ...
      this.gold(r, p, -1);
    } else this.debit(r, p, pending.monto, this.player(r, pending.dueñoId));
  ...
  roll(r, p) {
    ...
    r.fase = 'gestion';
    r.ultimaTirada = { id: randomUUID(), jugador: p.nombre, dados: dice, total: distance };
  ...
  land(r, p, position) {
    ...
    if (c.tipo === 'propiedad') {
      ...
      if (c.region === 'sur') {
        ...
        if (own) {
          if (!r.barreraProteccionista && !p.industriasCerradas && !p.turnosPerdidos) this.money(r, p, this.rent(r, c));
    ...
    switch (position) {
      ...
      case 12:
        ...
        else { const amount = this.dice() * 1000; this.debit(r, p, amount); this.log(r, `Fuga de capitales: $${amount}.`); }
        break;
      case 18:
        if (p.resguardoGolpe) p.resguardoGolpe = false;
        else this.money(r, p, -Math.max(0, p.dinero));
        break;
      ...
      case 30: this.money(r, p, 50); break;
      case 32: this.eachWallet(r, q => this.gold(r, q, -Math.min(q.oro, this.team(r, q).length))); break;
  ...
  buy(r, p, data) {
    ...
    if (data.comprar) { requireRule(p.dinero >= c.precio, 'Necesitas efectivo o un préstamo para comprar.'); this.money(r, p, -c.precio); c.dueño = p.id; this.log(r, `${p.nombre} compró ${c.nombre}.`); }
    else { this.money(r, p, c.precio); this.log(r, `${p.nombre} cobró $${c.precio} por la materia prima.`); }
    r.pendiente = null; r.fase = 'gestion';
  }
  build(r, p, data) {
  ...
  expropriate(r, p, data) {
    ...
    this.debit(r, p, investment, this.player(r, c.dueño)); this.money(r, p, -tax); this.transfer(r, c, p.id);
  ...
  card(r, p, type) {
    ...
    if (type === 'solidaridad') {
      ...
      if (rewards[c.id]) { this.money(r, p, rewards[c.id]); return; }
      switch (c.id) {
        ...
        case 11: for (const q of r.jugadores.filter(q => !q.enQuiebra)) this.money(r, q, 2000); break;
      ...
      switch (c.id) {
        ...
        case 6: { const q = active[(index + 1) % active.length]; q.posicion = 39; q.industriasCerradas = false; this.debit(r, q, this.interest(q)); break; }
  ...
  choose(r, p, data) {
    ...
    if (pending.efecto === 'pactoFmi' && data.opcion === 'aceptar') { p.sinPactos = true; this.money(r, p, 10000); }
    if (pending.efecto === 'dumping') this.transfer(r, this.sur(r, data.opcion), null);
  ...
  finishAuction(r) {
    ...
    if (buyer && !buyer.enQuiebra && buyer.dinero >= a.oferta.monto) { this.debit(r, buyer, a.oferta.monto, seller); this.transfer(r, c, buyer.id); }
    else { this.money(r, seller, a.base); this.transfer(r, c, null); }
  ...
  settleAbsent(r, p) {
    while (p.dinero < 0) {
      ...
      this.money(r, p, Math.floor(this.investment(r, asset) / 2)); this.transfer(r, asset, null);
    }
  }
  tick() {
    let changed = false;
