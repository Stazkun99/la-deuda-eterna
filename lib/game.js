...
class Game {
      ...
      if (event === 'responderVotoAlianza') this.vote(r, p, data);
        ...
        switch (event) {
          ...
          case 'pagarDeuda':
            ...
            { const amount = Math.min(5000, p.deudaPersonal); requireRule(p.dinero >= amount, 'No tienes efectivo suficiente.'); this.money(r, p, -amount, {accion:'Amortización',contraparte:'FMI'}); p.deudaPersonal -= amount; this.log(r, `${p.nombre} amortizó $${amount}.`); }
            break;
  ...
  pay(r, p, data) {
    ...
    if (data.usarOro) {
      ...
      this.interaction(r, p, 'Pago con oro', 'FMI', null, '1 lingote · ' + pending.motivo);
  ...
  land(r, p, position) {
    ...
    switch (position) {
      ...
      case 12:
        ...
        else { const amount = this.dice() * 1000; this.debit(r, p, amount, null, 'Fuga de capitales'); this.log(r, `Fuga de capitales: $${amount}.`); }
        break;
      case 18:
      ...
      case 30: this.money(r, p, 50, {accion:'Ayuda al desarrollo',contraparte:'BID'}); break;
